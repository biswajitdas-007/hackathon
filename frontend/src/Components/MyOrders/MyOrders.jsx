...
=======
  const params = useParams();
  const { list, getData, isAuth, listFilter, updateList } =
  ...
  const handleCancel = (id) => {
    axios.delete(`http://localhost:8000/book/${id}`).then((res) => {
      const del = list.filter((item) => id !== item._id);
      updateList(del);
      //  setList(del);
    });
              ...
              <>
                <div className={styles.box} key={item._id}>
                  ...
                  <div className={styles.insideBox}>
                    <b>SENDER NAME : </b> {item.senderName}
                  </div>
                  <br />
                  <div className={styles.insideBox}>
                    <b>SENDER ADDRESS : </b> {item.senderAddress}
                  </div>
                  <br />
                  <div className={styles.insideBox}>
                    <b>RECIEVER NAME : </b> {item.recieverName}
                  </div>
                  <br />
                  <div className={styles.insideBox}>
                    <b>RECIEVER ADDRESS : </b> {item.recieverAddress}
                  </div>
                  <br />
                  <div className={styles.insideBox}>
                    <b>PICKUP TIME : </b> {item.pickupTime}
                  </div>
                  <br />
                  <div className={styles.insideBox}>
                    <b>PREFERABLE TIME : </b> {item.preferableTime}
                  </div>
                  <br />
                  {/* <div><b>SizePrice:</b> {item.sizePrice}</div> */}
                  <div className={styles.butCont}>
                    ...
                    <button className={styles.but}>
                      {item.payment ? (
                        <Link
                          to={`/myorders/${item._id}`}
                          className={styles.payTxt}
                        >
                          Track
                        </Link>
                      ) : (
                    ...
                    >
                      <Link to={`/myorders`} className={styles.payTxt}>
                        Cancel Order
                      </Link>
                    </button>
                  </div>
...
>>>>>>> 2f3ab4b9020e1fa14a2895f19117898081243635
}
